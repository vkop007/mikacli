...
export class AudioEditorAdapter {
  ...
  async info(input: AudioInfoInput): Promise<AdapterActionResult> {
    ...
    if (!audioStream) {
      throw new MikaCliError("AUDIO_INFO_UNAVAILABLE", "Could not read audio stream information from the file.", {
        details: {
  ...
  async merge(input: AudioMergeInput): Promise<AdapterActionResult> {
    ...
    if (inputPaths.length < 2) {
      throw new MikaCliError("EDITOR_INVALID_ARGUMENT", "Merge needs at least two input audio files.", {
        details: {
  ...
  async fadeOut(input: AudioFadeInput): Promise<AdapterActionResult> {
    ...
    if (totalDuration === undefined) {
      throw new MikaCliError("AUDIO_INFO_UNAVAILABLE", "Could not determine audio duration for fade-out.", {
        details: {
          inputPath: input.inputPath,
        },
      });
    ...
    if (!Number.isFinite(start)) {
      throw new MikaCliError("EDITOR_INVALID_ARGUMENT", "Invalid fade-out start value.");
    }
  ...
  async volume(input: AudioVolumeInput): Promise<AdapterActionResult> {
    ...
    if (db === undefined) {
      throw new MikaCliError("EDITOR_INVALID_ARGUMENT", "Invalid volume change value.");
    }
  ...
  async split(input: AudioSplitInput): Promise<AdapterActionResult> {
    ...
    if (input.bySilence) {
        ...
        if (!segments || segments.length === 0) {
             throw new MikaCliError("EDITOR_COMMAND_FAILED", "No silence segments found to split by.", { details: { inputPath: input.inputPath } });
        }
  ...
  async eq(input: AudioEqInput): Promise<AdapterActionResult> {
    ...
    if (filters.length === 0) {
      throw new MikaCliError("EDITOR_INVALID_ARGUMENT", "Eq needs at least a non-zero bass or treble adjustment.");
    }
...
async function runAudioCommand(args: readonly string[]): Promise<void> {
  await new Promise<void>((resolvePromise, rejectPromise) => {
    const child = spawn(process.env.MIKACLI_FFMPEG_BIN || "ffmpeg", ["-y", "-hide_banner", "-loglevel", "error", ...args], {
      env: process.env,
...
async function runAudioAnalysisCommand(args: readonly string[]): Promise<{ stdout: string; stderr: string }> {
  return new Promise((resolvePromise, rejectPromise) => {
    const child = spawn(process.env.MIKACLI_FFMPEG_BIN || "ffmpeg", args, {
      env: process.env,
      stdio: ["ignore", "pipe", "pipe"],
    });
...
export function parseLoudnessReportOutput(stderr: string): {
  ...
  if (!jsonBlock) {
    throw new MikaCliError("AUDIO_LOUDNESS_REPORT_UNAVAILABLE", "Could not parse ffmpeg loudness analysis output.", {
      details: {
        stderr: stderr.trim() || null,
      },
    });
