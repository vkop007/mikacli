...
- Provider: `claude`
- Category: `llm`
- Command prefix: `mikacli llm claude`
- Aliases: none
- Auth: `cookies`
- Stability: `partial`
- Discovery: `supported`
...
- `mikacli llm claude login`
- `mikacli llm claude login --cookies ./claude.cookies.json`
- `mikacli llm claude text "Summarize this changelog"`
- `mikacli llm claude capabilities --json`

## Default Command

Usage:
```bash
mikacli llm claude [command]
```
...
Usage:
```bash
mikacli llm claude login [options]
```

Save the Claude session for future CLI use. With no auth flags, MikaCLI opens browser login by default
...
Usage:
```bash
mikacli llm claude status [options]
```
...
Usage:
```bash
mikacli llm claude text [options] <prompt...>
```
...
Usage:
```bash
mikacli llm claude image [options] <mediaPath>
```
...
Usage:
```bash
mikacli llm claude video [options] <prompt...>
```
...
Usage:
```bash
mikacli llm claude capabilities [options]
```
