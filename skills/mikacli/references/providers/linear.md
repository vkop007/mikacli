...
- Provider: `linear`
- Category: `developer`
- Command prefix: `mikacli developer linear`
- Aliases: none
- Auth: `cookies`
- Stability: `partial`
- Discovery: `supported`
...
- `mikacli developer linear login`
- `mikacli developer linear login --cookies ./linear.cookies.json`
- `mikacli developer linear me`
- `mikacli developer linear capabilities --json`

## Default Command

Usage:
```bash
mikacli developer linear [command]
```
...
Usage:
```bash
mikacli developer linear login [options]
```

Save the Linear web session for future CLI use. With no auth flags, MikaCLI opens browser login by default
...
Usage:
```bash
mikacli developer linear status [options]
```

Show the saved Linear session status

Options:

- `--account <name>`: Optional saved Linear session name to inspect

### `me`

Usage:
```bash
mikacli developer linear me [options]
```
...
Usage:
```bash
mikacli developer linear teams [options]
```
...
Usage:
```bash
mikacli developer linear projects [options]
```
...
Usage:
```bash
mikacli developer linear issues [options]
```
...
Usage:
```bash
mikacli developer linear issue [options] <id-or-key>
```
...
Usage:
```bash
mikacli developer linear create-issue [options]
```
...
Usage:
```bash
mikacli developer linear update-issue [options] <id-or-key>
```
...
Usage:
```bash
mikacli developer linear comment [options] <id-or-key>
```
...
Usage:
```bash
mikacli developer linear capabilities [options]
```
